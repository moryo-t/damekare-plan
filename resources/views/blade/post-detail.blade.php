    ...
    <div class="container my-5">
        @foreach ($items as $item) 
            <div class="d-flex justify-content-between flex-wrap">
                <h4>{{ $item->title }}</h4>
                <form action="/posts/destroy/{{ $item->id }}" id="post_remove">
                    @csrf
                    <button type="button" class="btn btn-danger" id="remove_button">投稿を削除</button>
                </form>
            </div>
            <div>
                <div>始発場所</div>
...
@section('script')
    <script src="{{ asset('js/remove-post.js') }}" defer></script>
@endsection
